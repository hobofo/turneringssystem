...
if(isset($_POST["turneringsid"])){
    ...
    // Sætter ind i puljer
    $results = mysqli_query($link,"DELETE FROM hbf_puljer WHERE turnerings_id = '$turneringsid'") or die(mysql_error());
    ...
    while($row = mysql_fetch_array($result)){
        ...
        $results = mysqli_query($link,"INSERT INTO hbf_puljer (turnerings_id,pulje_nr,spiller_id,point,kampe,type) values ('$turneringsid','$holdpulje','$spiller_id','$point','0','p')") or die(mysql_error());   
    }

    // Sætter ind i kampe
    $kampnr = $i = 0;
    $results = mysqli_query($link,"DELETE FROM hbf_kampe WHERE turnerings_id = '$turneringsid'") or die(mysql_error());
    $result = mysqli_query($link,"SELECT *  FROM `hbf_puljer` where turnerings_id = '$turneringsid' ORDER BY pulje_id DESC") or die(mysql_error());
    ...
    while($row = mysql_fetch_array($result)){
        ...
        while($rowp = mysql_fetch_array($result_partner)){
            ...
            if(mysql_num_rows($komp)< 1){
                    ...
                    $insert = mysqli_query($link,"INSERT INTO hbf_kampe (turnerings_id,hold1,hold2,rang1,rang2,type,kampnr,pulje,parameter) values ('$turneringsid','$spiller_id','$modstander','$rang1','$rang2','p','$kampnr','$pulje_nr','$i')")or die(mysql_error());
            }
        }
